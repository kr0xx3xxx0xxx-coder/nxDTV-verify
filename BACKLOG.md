...
### M292. 종결/재정의(2026-09-11, 방향 재검토) - LARGE-MATCHED-GROUP-RECORDSET-HASH-BLIND-SPOT-ALTERNATIVE - 대용량 "일치" 그룹 레코드셋 해시 검증 확장은 기각, 전수검증 안내로 방향 전환
...
- **2026-09-11 방향 재검토(채팅 논의, 지침/조사 실행 없음)**: 이 아이디어를
  기각한다. (1) 조합축 GROUP BY 자체가 이미 통계지문 우연일치 확률을
  낮추기 위한 설계라 별도 해시 확장은 이중 대응. (2) 개별/일괄검증은
  원래 통계검증이고, 대형 "일치" 그룹까지 전수 해시하는 것은 사실상
  전수검증 영역을 부분적으로 재구현하는 것 — 이미 별도 메뉴로 존재하는
  전수검증과 경계가 흐려짐. (3) 논리 일관성 문제: "통계가 숨길 수
  있다"는 전제를 받아들이면 불일치 그룹 내 "일치"로 보이는 개별
  레코드도 똑같이 해시해야 하는데 그 확장까지는 의도한 적 없음 —
  일관성 없는 부분 패치. (4) 이 프로젝트의 이관은 테이블 단위 통짜
  INSERT...SELECT(JOIN 없는 단순 1:1)라 국소적 행 뒤바뀜 같은 오류가
  날 메커니즘이 없고, 구조적 오류는 대체로 여러 축 중 하나에서 통계로
  걸림. 결론: 상한 확장/청크 해시 엔지니어링은 하지 않고, 이 규모의
  확실성이 필요하면 전수검증 메뉴를 안내하는 쪽으로 방향 전환(구현은
  미착수, 방향성 결론만 — 실제 안내 문구 추가는 별도 신규 항목으로
  필요 시 등록).
...
- `RunnerCapabilities` 4개 클래스(`{MSSQL,MySQL,Oracle,PostgreSQL}
  ...
  (M372에서 삭제보류로 유지 확정). 이 항목(M385)은 shim **파일** 자체의
  구조적 삭제만 대상으로 함.
